...
const NotificationItem = ({ notification, onAction, countFunc, nFunc }) => {
    const [loading, setLoading] = useState(false);
    const [actionTaken, setActionTaken] = useState(null);
    const [isStillPendingBooking, setIsStillPendingBooking] = useState(true);
    const [isStillPendingStation, setIsStillPendingStation] = useState(true);
    ...
    const handleApproveBooking = async () => {
      if (!notification.stationId || !notification.relatedId) {
        console.error("Missing stationId or bookingId");
        return;
      }
    ...
    const handleApproveStation = async () => {
      if (!notification.relatedId) {
        console.error("Missing stationId");
        return;
      }
    ...
    const handleMarkAsRead = async (id) => {
      try {
        ...
        countFunc((prev) => Math.max(prev - 1, 0));
      } catch (error) {
        console.error("Error marking notification as read:", error.message);
      }
    };
  
    // const showBookingActions = notification.title === "New Booking Request" && !actionTaken && isPending(notification.type,notification.relatedId);
    const showBookingActions = notification.title === "New Booking Request" && !actionTaken && isStillPendingBooking;
    const showStationActions = notification.title === "New Station Request" && !actionTaken && isStillPendingStation;
    
    useEffect(() => {
      const checkPending = async () => {
        try {
          if (notification.type === "station") {
            const res = await fetch(`${API_BASE_URL}/api/stations/${notification.relatedId}`, {
              headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
            });
            const data = await res.json();
            setIsStillPendingStation(data.status === "pending");
          } else if (notification.type === "booking" && notification.stationId) {
            ...
            const data = await res.json();
            setIsStillPendingBooking(data.status === "pending");
          }
        } catch (err) {
          console.error("Error checking pending status:", err);
          setIsStillPendingStation(false);
          setIsStillPendingBooking(false);
        }
      };
    ...
    return (
      <div
        className={`px-5 py-4 hover:bg-gray-50 transition-colors relative ${
            ...
            {(showBookingActions || showStationActions) && (
              <div className="flex gap-2 mt-3 mb-2">
                <button
                  ...
                  disabled={loading}
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-green-500 hover:bg-green-600 disabled:bg-green-300 text-white text-xs font-medium rounded-lg transition-colors"
                >
                ...
                <button
                  ...
                  disabled={loading}
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-red-500 hover:bg-red-600 disabled:bg-red-300 text-white text-xs font-medium rounded-lg transition-colors"
                >
